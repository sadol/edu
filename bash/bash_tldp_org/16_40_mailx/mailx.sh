...
abuser="$(whois -h whois.abuse.net "$1")"
# only one address ,idk the exact format of the `whois' response
abuser_mail="${abuser%% *}"
#     Can even lookup multiple spammers with this, i.e."
#     whois -h whois.abuse.net $spamdomain1 $spamdomain2 . . .

# you must set up your `mailx' properly using ~/.mail.rc first, and then...
body="You are ugly abuser and I do not like you at all."
subject="Ugly abuser."
mailx -v -s "$subject" "$abuser_mail" <<< "$body"
