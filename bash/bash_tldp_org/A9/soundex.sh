...
assign_value () {               #  Assigns numerical value to letters of name.
    val1=bfpv                    # 'b,f,p,v' = 1
...
prefix=${prefix0^^}                          # Uppercase 1st letter of soundex.
((char_pos++))                 # Bump character position to 2nd letter of name.
name1=${name:$char_pos}
...
char1=${prefix,,}                           # First letter of name, lowercased.
assign_value $name
s1=$value
assign_value $name1
s2=$value
...
[[ "$s1" -ne "$s2" || -z "$s3" ]] && suffix=$s2 || suffix=${s2:$char_pos} # "simplifying" of sorts
# ++++++++++++++++++++++ end Exception Patch ++++++++++++++++++++++++++++++
