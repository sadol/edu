...
const (                                                        // magic numbers
    //EPSILON = 1e-15                   // accuracy threshold of PI approximation
    NUMGOROUTINES = 10                         // number of goroutines employed
    NUMFACTORS = 20          // number of elements for return from subgoroutine
)
...
/*
// checks if significance threshold has been exceeded
func checkEpsilon(first, second float64) (abort bool) {
    if math.Abs(first - second) >= EPSILON { abort = true }
    return
}
*/
